...
sub new
{
    ...
    unless (defined $params{uniqueid}) {
        &::printEvent("HLSTATSX", "HLstats_Player->new(): missing uniqueid", 1);
        return undef;
    }
    ...
    &::printEvent("","Created new player object " . $self->getInfoString(),4);
    return $self;
}
...
sub check_history
{
    ...
    if ($self->{is_bot} || (( $self->{userid} < 1 && $::g_servers{$self->{server}}->{play_game} != CS2()) ||  ($self->{userid} < 0 && $::g_servers{$self->{server}}->{play_game} == CS2()))) {
        $is_bot = 1;
    }  
...
sub flushDB
{
    ...
    if ($::g_stdin == 0 && ($self->{userid} > 0 || $::g_servers{$server_address}->{play_game} == CS2()) ) {
        # Update live stats
        my $query = "
            UPDATE
                hlstats_Livestats
    ...
    &::printEvent("","Updated player object " . $self->getInfoString(),2,1);
...
sub getAddress
{
    ...
    if ($self->{address}) {
        ...
        &::printEvent("RCON","rcon_getaddress for Player $slot_name",1,1);
...
print Dumper($result);
        ...
        if (defined $result->{Address} && $result->{Address} ne "") {
            ...
            &::printEvent("RCON", "Got Address $self->{address} for Player $self->{name}", 1,1);
        }
    }
    ...
    if ($haveAddress > 0)
    {
        ...
        &::printEvent("DEBUG", "Updated IP for ".$self->{playerid}." to ".$self->{address},2);
...
sub geoLookup
{
    ...
    if ($ip_address ne "")
    {
        ...
        if ($::g_geoip_binary > 0) {
            ...
            eval { $geoCity = $::g_gi->city( ip => $ip_address ); };
            $geoCity = undef if $@;
...
sub getRank
{
    ...
    if ($::g_ranktype ne "kills")
    {
        ...
        {
            &::printEvent("ERROR","Attempted to get rank for uninitialized player \"".$self->{name}."\"",2);
            return 0;
        }
