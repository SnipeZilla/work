...
sub flushDB {
    ...
    if ($is_bot) {
        ...
        ::exec_cache("player_flushdb_player_1", $query,
            $add_connect_time, $name, $kills, $deaths, $suicides,
            $headshots, $shots, $hits, $teamkills, $::ev_unixtime, $playerid
        );
    } else {
        my $query = q{
            UPDATE hlstats_Players
        ...
        ::exec_cache("player_flushdb_player_2", $query,
            $add_connect_time, $name, $clan, $kills, $deaths, $suicides, $skill,
            $headshots, $shots, $hits, $teamkills, $::ev_unixtime, $last_skill_change,
            $death_streak = $self->{death_streak}, $death_streak,
            $kill_streak  = $self->{kill_streak},  $kill_streak,
            $playerid
        );
    ...
    if (!$is_stdin && ($self->{userid} > 0 || $::g_servers{$server_address}->{play_game} == CS2())) {
        ...
        ::exec_cache("player_flushdb_livestats", $query,
            $address, $steamid, $name, $team,
            $map_kills, $map_deaths, $map_suicides, $map_headshots,
            $map_shots, $map_hits, $self->{is_dead}, $self->{has_bomb},
            $self->{ping}, $self->{connect_time}, $self->{session_skill}, $skill,
            $playerid
        );
    }
